...
var KChart = React.createClass({
	...
	componentDidMount:function(){
		...
		InteractionManager.runAfterInteractions(() => {
		 	objectAction.loadPriceData(me.props.code,me.props.cycle);
		});	
	},
	...
	onCycleSelect:function(cycle,name){	
		objectAction.loadPriceData(this.props.code,cycle);
	},
	_offset:0,
	_isZoom:false,
	onScroll:function (e) {
		...
		if(e.nativeEvent.contentOffset.y>this._offset){
			...
			if(e.nativeEvent.contentOffset.y>=0&&e.nativeEvent.contentOffset.y<20){
				if(this._isZoom){
			      	Animated.parallel([
					Animated.timing(
			         	this.state.top,         // Auto-multiplexed
...
var RecommentStock=React.createClass({
	...
	loadData:function (argument) {
		myStockAction.loadRecommendStock();
	},
	render: function() {
		var recoList=[];
		        ...
		        <ScrollableTabView 
		          ...
		          ref={viewPager => { this._viewPager = viewPager; }}>
		            <View tabLabel={"K线"}  style={{flex:1}}>
				    	<KChart code={''} name={''} cycle={'day'}/>
		            </View>
		            <View tabLabel={"排行"} style={{flex:1}}>             
		           		
		            </View>
		            <View tabLabel={"推荐"} style={{flex:1}}>             
		           		<RecommentStock ref={(n)=>this._recoStock=n}/>
		            </View>
		        </ScrollableTabView >
			</View>
		);
