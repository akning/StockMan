...
var Item = React.createClass({
	onSelect:function () {		
		this.props.onSelect&&this.props.onSelect(this.props.data.code,this.props.data.name);
	},
	render: function() {
		return (
			<TouchableWithoutFeedback onPress={this.onSelect}>
				<View style={[styles.item,{backgroundColor:this.props.data.selected? ColorConfig.baseColor:'#d3d3d3'}]}>
					<Text style={{color:this.props.data.selected?'white':'black'}}>{this.props.data.name}</Text>
				</View>
			</TouchableWithoutFeedback>
		);
	}
...
var ItemList = React.createClass({
	...
	onChange:function(state){ 
		if(state.data[this.props.cate]==undefined)
			return;
		this.setState(state);
	},
